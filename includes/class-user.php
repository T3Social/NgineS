...
class User {
        ...
        if($conversation_id == null) {
            ...
            foreach($conversation['recipients'] as $recipient) {
                if($this->blocked($recipient['user_id'])) {
                    modal("MESSAGE", __("Message"), __("You aren't allowed to message")." ".($system['show_usernames_enabled'])? $recipient['user_name']: $recipient['user_firstname']." ".$recipient['user_lastname']);
                }
            }
     ...
     * @return array
     */
    public function get_blogs_category($category_id) {
        global $db;
        $get_category = $db->query(sprintf("SELECT * FROM blogs_categories WHERE category_id = %s", secure($category_id, 'int') )) or _error("SQL_ERROR_THROWEN");
     ...
     * @return array
     */
    public function get_market_category($category_id) {
        global $db;
        $get_category = $db->query(sprintf("SELECT * FROM market_categories WHERE category_id = %s", secure($category_id, 'int') )) or _error("SQL_ERROR_THROWEN");
        if($get_category->num_rows == 0) {
            return false;
        }
        return $get_category->fetch_assoc();
     ...
     * @return array
     */
    public function get_offers_category($category_id) {
        global $db;
        $get_category = $db->query(sprintf("SELECT * FROM offers_categories WHERE category_id = %s", secure($category_id, 'int') )) or _error("SQL_ERROR_THROWEN");
        if($get_category->num_rows == 0) {
            return false;
        }
        return $get_category->fetch_assoc();
     ...
     * @return array
     */
    public function get_jobs_category($category_id) {
        global $db;
        $get_category = $db->query(sprintf("SELECT * FROM jobs_categories WHERE category_id = %s", secure($category_id, 'int') )) or _error("SQL_ERROR_THROWEN");
        if($get_category->num_rows == 0) {
            return false;
        }
        return $get_category->fetch_assoc();
    ...
    public function settings($edit, $args) {
        ...
        switch ($edit) {
            ...
            case 'privacy':
                ...
                /* update user */
                $db->query(sprintf("UPDATE users SET user_chat_enabled = %s, user_privacy_newsletter = %s, ".$poke_statement.$gifts_statement." user_privacy_wall = %s, user_privacy_gender = %s, user_privacy_birthdate = %s, user_privacy_relationship = %s, user_privacy_basic = %s, user_privacy_work = %s, user_privacy_location = %s, user_privacy_education = %s, user_privacy_other = %s, user_privacy_friends = %s, user_privacy_photos = %s, user_privacy_pages = %s, user_privacy_groups = %s, user_privacy_events = %s WHERE user_id = %s", secure($args['privacy_chat']), secure($args['privacy_newsletter']), secure($args['privacy_wall']), secure($args['privacy_gender']), secure($args['privacy_birthdate']), secure($args['privacy_relationship']), secure($args['privacy_basic']), secure($args['privacy_work']), secure($args['privacy_location']), secure($args['privacy_education']), secure($args['privacy_other']), secure($args['privacy_friends']), secure($args['privacy_photos']), secure($args['privacy_pages']), secure($args['privacy_groups']), secure($args['privacy_events']), secure($this->_data['user_id'], 'int') )) or _error("SQL_ERROR_THROWEN");
                break;

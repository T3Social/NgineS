...
try {
	...
	switch ($_GET['type']) {
		case 'wallet':
			...
			/* update user balance*/
			$balance = $user->_data['user_wallet_balance'] - $_POST['amount'];
			$db->query(sprintf("UPDATE users SET user_wallet_balance = %s WHERE user_id = %s", secure($balance), secure($user->_data['user_id'], 'int') )) or _error("SQL_ERROR_THROWEN");
			/* wallet transaction */
